...
b8 platform_startup(platform_state* plat_state, const char* application_name,
    ...
    // retrive the connection object from the display
    ...
    // retrive data from the x server
    ...
    // generate an xid fro the widnow to be created
    ...
    // values to be sent over XCB {bg_color, events}
    u32 value_list[] = {state->screen->black_pixel, event_mask};

    // create a window
    xcb_void_cookie_t cookie =
        xcb_create_window(state->connection, XCB_COPY_FROM_PARENT,
                          state->window, state->screen->root, x, y, width,
